...
import sys
sys.path.append(r"D:\IdeaProjects\seeyon\cap+\Common") #跨目录调用需要配置路径
...
class RunCase(unittest.TestCase):# 继承unittest.TestCase
    ...
    def test_case(self):
        ...
        #执行路径
        casespath = r"D:\IdeaProjects\seeyon\cap+\TestCases" #执行前需检验路径和文件
        ...
        # #执行并且输出测试报告txt格式
        # with open(r"D:\IdeaProjects\seeyon\cap+\TestResults\TestResult.txt", "w", encoding="utf-8") as result_txt: #测试报告路径
        ...
        # #执行并且输出测试报告html格式
        # with open(r"D:\IdeaProjects\seeyon\cap+\TestResults\TestResult.html", "wb") as result_html: #测试报告路径
