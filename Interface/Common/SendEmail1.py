...
#发送附件邮件配置
def email_appendix(title, from_name, from_address, to_address,serverport, serverip, username, password):
    mg = MIMEMultipart()
    mg["Subject"] = Header(title,"utf-8") #邮件标题
    mg["To"] = Header(to_address,"utf-8") #收件地址to_address为string格式
...
def sendemail():
    ...
    #服务器参数设置
    config = {
        "from": "775636762@qq.com",
        "from_name": "Python接口自动化测试框架:",
        "to": adresses[0],
        "serverip": "smtp.qq.com",
        "serverport": "465",
        "username": "775636762@qq.com",
        "password": "txhattvlgxzxbfdc"  # QQ邮箱的SMTP授权码
    }
    ...
    # #调用附件邮箱配置
    # email_appendix(title, config["from_name"], config["from"], config["to"], config["serverport"], config["serverip"],
    #                config["username"], config["password"])
