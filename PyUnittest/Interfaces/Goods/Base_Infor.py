...
#获取商品基本信息地址
search_url = "%s/rest/goods/index" % BasicDatas.test_url_mall

def base_infor(goods_id):
    data = {
    ...
    if response_search["code"] == "1000":
        ...
        print("%s：商品基本信息获取成功！！！" % response_search["data"]["goods_name"])
    else:
        message = response_search["message"]
        print("%s：商品基本信息获取失败！！！原因为：%s" % (goods_id,message))

if __name__ == "__main__":
    base_infor("15657462370390000")