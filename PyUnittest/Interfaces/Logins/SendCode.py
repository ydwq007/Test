...
#接口地址
send_url = "%s/portal.php" % BasicDatas.test_url_chome
...
def sendcode(param,sendby = "login",type = "mobile"): #参数为参数（手机或邮箱），验证码类型（可不传，默认为登录），验证码方式（可不传，默认为手机）
    ...
    #请求接口
    request_send = requests.get(send_url, data=datas, headers=BasicDatas.headers, verify=False) #调用接口（url,参数，类型）#verify=False 跳过认证
    ...
    # print("-----------打印请求数据如下：--------------")
    print("获取验证码地址为：%s" % send_url)
    print("获取验证码请求参数为：\n%s" % json_util.dumps(data,ensure_ascii=False,indent=4)) #打印请求数据，以json格式输出
